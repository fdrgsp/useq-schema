...
class MDAEventBuilder(EventBuilder[MDAEvent]):
    ...
    def __call__(
        ...
        if context:
            # Note: sequence field typed as v1.MDASequence, so we pass None for now
            # to maintain compatibility. The context is still available via iter_axes.
            contributions.append(("", {"sequence": None}))
...
class MDASequence(MultiAxisSequence[MDAEvent]):
    ...
    @property
    def grid_plan(self) -> AxisIterable[Position] | None:
